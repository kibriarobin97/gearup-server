...
const createRentalOrder = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await rentalOrderService.createRentalOrder(customerId as string, payload);
...
const getMyRentalOrders = catchAsync(async (req: Request, res: Response) => {
  const customerId = req.user?.id;
  const result = await rentalOrderService.getMyRentalOrders(customerId as string);
